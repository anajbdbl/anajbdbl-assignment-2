...
def generate_data():
    global kmeans_instance, currentK, hasConverged
    kmeans_instance = None
    currentK = None
    hasConverged = False
    num_points = 100
...
def run_kmeans_step():
    ...
    if step == 0 or kmeans_instance is None or k != currentK:
    ...
    if not hasConverged:
        hasConverged = not kmeans_instance.step(data)
    ...
    print(f"Step result: {len(result['centroids'])} centroids, {len(result['labels'])} labels, Converged: {hasConverged}")
